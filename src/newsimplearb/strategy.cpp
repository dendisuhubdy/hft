...
Strategy::Strategy(const libconfig::Setting & param_setting, const libconfig::Setting & ticker_setting, const TimeController& tc, std::unordered_map<std::string, std::vector<BaseStrategy*> >*ticker_strat_map, Contractor& ct, Sender* sender, Sender* ordersender, const std::string & mode, bool no_close_today)
  ...
  try {
    ...
    ticker_size = ticker_setting["contract_size"];
...
void Strategy::UpdateTicker() {
    ...
    if (v.size() < 2) {
      ...
      for (auto i:m_ct.GetAllTick()) {
        PrintVector(m_ct.GetTicker(i));
      }
      return;
    }
    main_ticker = v[1];
...
void Strategy::DoOperationAfterUpdateData(const MarketSnapshot& shot) {
  if (main_record && shot.ticker == main_ticker) {
    ...
    double slip_loss =  -(next_price - signal_price)*main_record*ticker_size;
  ...
  if (hedge_record && shot.ticker == hedge_ticker) {
    ...
    double slip_loss =  -(next_price - signal_price)*hedge_record*ticker_size;
