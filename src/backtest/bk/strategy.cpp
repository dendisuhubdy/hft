...
void Strategy::CalParams() {
  ...
  std = sqrt(std);
  FeePoint main_point = caler->CalFeePoint(main_ticker, GetMid(main_ticker), 1, GetMid(main_ticker), 1);
  FeePoint hedge_point = caler->CalFeePoint(main_ticker, GetMid(main_ticker), 1, GetMid(main_ticker), 1);
...
bool Strategy::Close(bool force_flat) {
  ...
  if (order_map.empty()) {
    ...
    Order* o = NewOrder(main_ticker, close_side, abs(pos), false, false, force_flat ? "force_flat" : "close");  // close
    o->Show(stdout);
    HandleTestOrder(o);
    if (mode == "real") {
      double this_round_pnl = caler->CalNetPnl(main_ticker, avgcost_map[main_ticker], abs(pos), o->price, abs(pos), close_side) + caler->CalNetPnl(hedge_ticker, avgcost_map[hedge_ticker], abs(pos), hedge_price, abs(pos), pos_side);
      Fee main_fee = caler->CalFee(main_ticker, avgcost_map[main_ticker], abs(pos), shot_map[main_ticker].  bids[0], abs(pos));
      Fee hedge_fee = caler->CalFee(hedge_ticker, avgcost_map[hedge_ticker], abs(pos), hedge_price, abs(pos));
...
void Strategy::DoOperationAfterFilled(Order* o, const ExchangeInfo& info) {
  if (strcmp(o->contract, main_ticker.c_str()) == 0) {
    ...
    Order* order = NewOrder(hedge_ticker, (o->side == OrderSide::Buy)?OrderSide::Sell : OrderSide::Buy, info.trade_size, false, false, "");
    order->Show(stdout);
